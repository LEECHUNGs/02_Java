...
public class ByteService {
	...
	/* *****************************************
	 * IO 관련된 코드는 IOExeption이 발생될 가능성이 높다!
	...
	public void fileByteOutput() {
		...
		try{
			...
			// -> 2byte 범주의 문자(영어, 숫자, 기본 특수문제 제외)는 1byte 기반 스트림 통과 시
			...
			// System.nanoTime() : 1970.01.01. 09:00:00 qnxj
			...
			// 사용 완료한 Stream을 제거(닫기/메모리 반환)하는 코드 작성
	...
	public void bufferedFileByteOutput() {
		...
		/* BufferOutputStream / BufferedInputStream
		 * 
	...
	public void fileByteInput() {
		...
		try{
			...
			// 자바에서 byte는 정수형
			// -> 다루기 힘들기에 정수 기본형인 int로 변환해서 사용
			
			int value = 0; // 읽은 바이트값을 저장할 변수
			
			StringBuilder sb = new StringBuilder();
			
			while(true) {
				value = fis.read(); // 다 읽어왔으면 break
	...
	public void fileByteInput2() {
		...
		try{
			...
			//		   -> String 생성 시 배열 요소 2개씩 묶어와 한 글자로 해석해
			//		   		깨지는 글자 없이 문자열로 변환
	...
	/**
	 * BufferedInputStream 보조 스트림을 이용한 성능 향성
	 */
	 ...
	 * 
	 * 3) 읽어온 내용을 같은 위치에 "파일명.copy" 이름으로 출력
	 * 		: FileOutputStream + BufferedStream
	 * 
	...
	public void fileCopy() {
		...
		// 스트림 참조변수 선언
		BufferedReader br = null; // 문자열 입력용 Stream
		...
		try{
			...
			bos = new BufferedOutputStream(fos); // 출력 보조용 스트림
