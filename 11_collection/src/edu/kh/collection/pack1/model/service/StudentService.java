...
public class StudentService {
	...
	public void sortByName() {
		...
		// Comparator 인터페이스의 static 메서드인 comparing()을 사용해 Comparator를 생성
		// -> comparing()를 주어진 key를 기반으로 객체를 비교
		// Student::getName : 메서드 레퍼런스(Method Reference)
		// -> Student 클래스의 getName() 메서드를 가리키는 것.
		// -> 이 메서드를 비교의 키로 사용해 각 Student 객체를 비교하고 정렬
		// --> Comparator.comparing(Student::getName)은 이름(name)을 기준으로
		// 		학생(Student) 객체를 비교하는 Comparator 생성
		
		
		// 이름에 따라 정렬
		Collections.sort(studentList, nameCoparator);
